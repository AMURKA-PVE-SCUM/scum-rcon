...
* **Управление транспортом и база данных `SCUM.db`**:
  * `BringVehicle <VehicleId> [Player]` — мгновенная доставка транспорта к игроку: для загруженных авто — без перемещения игрока, для спящих — через безопасный неблокирующий асинхронный стриминг.
...
| `BringVehicle <VehicleId> [Player]` | Безопасная доставка транспорта к игроку (для загруженных авто — без движения игрока, для спящих — через асинхронный стриминг) с обновлением `SCUM.db`. |
...
### v1.1.0
  ...
  * **Коррекция высоты**: транспорт теперь появляется прямо на уровне земли перед персонажем (без зависания в воздухе).
  * **Точная фильтрация акторов**: многоуровневый поиск через сверку именованных свойств `EntityId`/`VehicleId` и координат в базе `SCUM.db`.
* **Асинхронный пайплайн команд (`dispatch_command_async`)**:
  * RCON команды, требующие нескольких кадров движка (стриминг, левел-лоадинг), выполняются полностью асинхронно без фриза сервера.
* **Обновлены готовые сборки**:
  * Обновлён `main.dll` и готовый архив `SCUM-RCON-Win64.zip`.
